...
export default function State() {
    ...
    useEffect(() => {
      ...
      const stateTitle =  data.map((item) => (<option value={item.id}>{item.state_title_en}</option>))
  const id = Number(window.location.pathname.replace('/cities/update/',''))
    ...
    const handleSubmit =  (event) => {
      ...
      formData.append("city_image", city_image);
        ...
        axios.post(`https://test.emkanfinances.net/api/city/update/${id}`, formData, {"Content-Type": "multipart/form-data"})
      .then((response) => {
    ...
    <div className="product">
      ...
      <div className="newUserItem">
        ...
        <div className="addProductItem">
          ...
          <input type="text" placeholder="City Title(Arabic)"  name="city_name_ar" value={city_name_ar} onChange={(e) => setcity_name_ar(e.target.value) }  />
        </div>
        <div className="addProductItem">
          <label>City Title(English)</label>
          <input type="text" placeholder="City Title(English)"  name="city_name_en" value={city_name_en} onChange={(e) => setcity_name_en(e.target.value) }  />
        </div>
        <div className="addProductItem">
          <label>Stat's Image</label>
          <input type="file" id="file" multiple onChange={(e) => setcity_image(e.target.files.item(0))} />
        </div>
